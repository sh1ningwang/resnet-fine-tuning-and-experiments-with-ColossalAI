...
@torch.no_grad()
def evaluate(
    model: nn.Module, test_dataloader: DataLoader, coordinator: DistCoordinator
) -> float:
    model.eval()
    correct = torch.zeros(
...
def main():
    ...
    parser.add_argument(
        "-i", "--interval", type=int, default=5, help="interval of saving checkpoint"
    )
    parser.add_argument(
        "--target_acc",
        type=float,
    ...
    # Prepare Dataloader
    # ==============================
    train_dataloader, test_dataloader = build_dataloader(128, coordinator, plugin)
    ...
    if args.resume >= 0:
        ...
        booster.load_optimizer(
            optimizer, f"{args.checkpoint}/optimizer_{args.resume}.pth"
        )
        booster.load_lr_scheduler(
            lr_scheduler, f"{args.checkpoint}/lr_scheduler_{args.resume}.pth"
        )
    ...
    for epoch in range(start_epoch, NUM_EPOCHS):
        ...
        if args.interval > 0 and (epoch + 1) % args.interval == 0:
            ...
            booster.save_optimizer(
                optimizer, f"{args.checkpoint}/optimizer_{epoch + 1}.pth"
            )
            booster.save_lr_scheduler(
                lr_scheduler, f"{args.checkpoint}/lr_scheduler_{epoch + 1}.pth"
            )

    accuracy = evaluate(model, test_dataloader, coordinator)
    if args.target_acc is not None:
        assert (
            accuracy >= args.target_acc
        ), f"Accuracy {accuracy} is lower than target accuracy {args.target_acc}"
